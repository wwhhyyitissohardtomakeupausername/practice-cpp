...
int main() {
    ...
    std::cout << "without return type annotation: " << func2(10, 20) << std::endl;

    // 规则二：如果没有参数，可以省略 (parameters)
    auto func3 = []() { return 999; };
    std::cout << "without parameters: " << func3() << std::endl; // 注意这里调用时是空的 ()

    // 规则三：如果既没参数，又能推断返回值，那就只剩 [] 和 {body}
    // (对应图片中的：[]{ ++global_x; })
    // 注意：这里的 `[]` 是空的，所以它【无法】访问外面的 a 和 b！
    auto func4 = [] { return 100; }; 
    std::cout << "without capture list: " << func4() << std::endl;
    ...
    std::cout << "[&]: " << func6() << std::endl; // 输出 70
    std::cout << "a outside: " << a << std::endl; // 输出 50
    ...
    std::cout << "[=, &b]: " << func7() << std::endl; // 输出 30
